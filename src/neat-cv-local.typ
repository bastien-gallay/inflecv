// neat-cv-local.typ
// Local fork of neat-cv v0.4.0 with first-page-only sidebar support
// Source: https://github.com/UntimelyCreation/typst-neat-cv
//
// MODIFICATION: Separated page 1 (with sidebar) from pages 2+ (full width)
// Usage: Use cv-page-one() for first page, then cv-continued() for rest
...
#let __st-side-width = state("side-width")
#let __st-heading-font = state("heading-font")
...
// ---- Visual Elements ----
#let level-bar(level, max-level: 5, width: 3.5cm, accent-color: none) = {
  context {
...
// ---- Entry Blocks ----
#let entry(title: none, date: "", institution: "", location: "", description) = {
        ...
        [
          #for publication in publications-by-year.at(year) {
            block([
              ...
              #__format-publication-entry(publication, highlight-authors, max-authors)
            ])
          }
        ],
      )
...
// ---- CV Setup (must be called first) ----
// Sets up document, fonts, and stores configuration in state
#let cv-setup(
  author: (:),
  ...
  context {
    ...
    __st-heading-font.update(heading-font)
  }
...
#let cv-page-one(
  ...
  context {
    ...
    let heading-font = __st-heading-font.final()
    let accent-color = theme.accent-color
    let header-color = theme.header-color
    ...
    block(
      ...
      #align(center)[
        ...
        #v(-0.5em)
        #text(size: 0.95em, fill: luma(200), weight: "regular")[
          #smallcaps(position)
        ]
      ]
    ]
...
// ---- Pages 2+: Full Width Content ----
// Use after pagebreak() for content that spans the full page width
#let cv-continued(content) = {
  context {
    let theme = __st-theme.final()
    let heading-font = __st-heading-font.final()
    let accent-color = theme.accent-color
...
// ---- Legacy: Original CV function (sidebar on all pages) ----
#let cv(
  author: (:),
  profile-picture: none,
  accent-color: rgb("#408abb"),
